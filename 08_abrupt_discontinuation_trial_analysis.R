...
#
# Analysis: Abrupt discontinuation versus continuation
#
# Purpose:
...
# 0. Setup ----------------------------------------------------------------------
#
# Load packages, define common covariate lists, set reproducibility options, and
...
#   dose_reduction_date    Date of dose reduction, where relevant
...
#   3. Create continuation and abrupt discontinuation clones.
...
abrupt_discontinuation_stroke_trials <- function(long_format, time_period, full_sample=FALSE) {
  ...
  ################################################################################
  # Abrupt Discontinuation Arm 
  ################################################################################
  # censor at earliest of:
  # 1. reinitiation
  # 2. end of grace period if failure to discontinue
  # 3. dose reduction date
  abrupt_discontinuation_arm <- long_format %>%
    mutate(treatment = "discontinuation") %>%
    group_by(patid) %>%
  ...
  abrupt_discontinuation_arm <- abrupt_discontinuation_arm %>%
  ...
  abrupt_discontinuation_arm <- abrupt_discontinuation_arm %>%
    mutate(censoring_date = pmin(no_discontinuation_date, dose_reduction_date, reinitiation_date, na.rm=TRUE))
  ...
  abrupt_discontinuation_arm <- abrupt_discontinuation_arm %>%
    ...
    mutate(dose_reduction_censor = ceiling((as.numeric(dose_reduction_date - T0) + 1)/ 28)) %>%
    mutate(dose_reduction_censor = ifelse(t < dose_reduction_censor, 0, 1)) %>%
    ...
    mutate(no_discontinuation_censor = ifelse(is.na(no_discontinuation_censor), 0, no_discontinuation_censor),
           dose_reduction_censor = ifelse(is.na(dose_reduction_censor), 0, dose_reduction_censor),
  ...
  abrupt_discontinuation_arm <- abrupt_discontinuation_arm %>%
    mutate(reinitiation_censor = ifelse(reinitiation_censor == 1 & no_discontinuation_censor == 1 |
                                          reinitiation_censor == 1 & dose_reduction_censor == 1, 0, reinitiation_censor))
  ...
  abrupt_discontinuation_arm <- abrupt_discontinuation_arm %>%
    ...
    mutate(dose_reduction_censor = ifelse(outcome_month == censoring_month & 
                                               !is.na(outcome_month) & !is.na(censoring_month) &
                                               outcome_date < censoring_date & t == outcome_month,
                                             0, dose_reduction_censor)) %>%
  ...
  grace_period_data <- abrupt_discontinuation_arm %>%
    filter(t == 7) %>%
  ...
  ps_model_discontinuation <- glm(remain_uncensored ~ age_T0 + gender + months_since_diagnosis +
                                    ...
                                    copd + heart_hypertension + angina + 
                                    # baseline medications
                                    ace_inhibitors_baseline  + antidepressants_baseline +
                                    antiepileptics_baseline + antiplatelets_baseline + anxiolytics_baseline +
                                    arbs_baseline + beta_blockers_baseline + calcium_blockers_baseline +
  ...
  not_discontinued_data <- abrupt_discontinuation_arm %>%
    filter(t < 7) %>%
    filter(lag(discontinued) == 0  | is.na(lag(discontinued)))
  
  not_discontinued_data <- not_discontinued_data %>%
    mutate(remain_uncensored = ifelse(dose_reduction_censor == 1, 0, 1))
  
  
  ps_model_discontinuation_dose_reduction <- glm(remain_uncensored ~ t + t_squared + age_T0 + gender + months_since_diagnosis +
                                                      # baseline conditions
                                                      angina_baseline + anxiety_baseline + atrial_fibrillation_baseline +
                                                      cancer_baseline + copd_baseline + depression_baseline + diabetes_baseline +
                                                      heartdisease_baseline + heart_failure_baseline + heart_hypertension_baseline +
                                                      ...
                                                      venous_thromboembolism_baseline + mi_baseline + 
                                                      # time-varying conditions
                                                      atrial_fibrillation + cancer + depression +
                                                      diabetes + heart_failure + heart_hypertension +
  ...
  not_discontinued_data$prob_uncensored <- predict(ps_model_discontinuation_dose_reduction, type="response")
  ...
  not_discontinued_data$unstabilized_weight_dose_reduction <- 1/not_discontinued_data$cum_prob_uncensored
  print("discontinuation no dose reduction unstabilized weights")
  print(summary(not_discontinued_data$unstabilized_weight_dose_reduction))
  
  baseline_ps_model_discontinuation_dose_reduction <-  
  ...
  not_discontinued_data$treatment_allocation_numerator <- predict(baseline_ps_model_discontinuation_dose_reduction, type="response")
  ...
  not_discontinued_data <- not_discontinued_data %>%
    mutate(stabilized_weight_dose_reduction = cum_prob_numerator/cum_prob_uncensored)
  print("discontinuation: no dose reduction stabilized weights")
  print(summary(not_discontinued_data$stabilized_weight_dose_reduction)) 
  
  not_discontinued_data <- not_discontinued_data %>%
    dplyr::select(patid, t, stabilized_weight_dose_reduction, unstabilized_weight_dose_reduction)
  
  abrupt_discontinuation_arm <- abrupt_discontinuation_arm %>%
    left_join(not_discontinued_data, by=c("patid", "t"))
  
  # reinitiation weights, which are only applied to those who have discontinued  
  reinitiation_weight_data <- abrupt_discontinuation_arm %>%
  ...
  abrupt_discontinuation_arm <- abrupt_discontinuation_arm %>%
    ...
    fill(
      ...
      stabilized_weight_dose_reduction,
      unstabilized_weight_dose_reduction,
  ...
  abrupt_discontinuation_arm <- abrupt_discontinuation_arm %>%
    mutate(stabilized_weight_no_discontinuation = ifelse(is.na(stabilized_weight_no_discontinuation), 1, 
           ...
           unstabilized_weight_no_discontinuation = ifelse(is.na(unstabilized_weight_no_discontinuation), 1,
                                                           unstabilized_weight_no_discontinuation),
           stabilized_weight_dose_reduction = ifelse(is.na(stabilized_weight_dose_reduction), 1,
                                                        stabilized_weight_dose_reduction),
           unstabilized_weight_dose_reduction = ifelse(is.na(unstabilized_weight_dose_reduction), 1,
                                                          unstabilized_weight_dose_reduction),
  ...
  abrupt_discontinuation_arm <- abrupt_discontinuation_arm %>%
    mutate(stabilized_weight_untruncated = stabilized_weight_no_discontinuation *
             stabilized_weight_dose_reduction * stabilized_weight_reinitiation,
           unstabilized_weight_untruncated = unstabilized_weight_no_discontinuation *
             unstabilized_weight_dose_reduction * unstabilized_weight_reinitiation)
  
  # truncate the stabilized weights at the 99th percentile
  quantile_cutoff <- quantile(abrupt_discontinuation_arm$stabilized_weight_untruncated, 0.99, na.rm=TRUE)
  abrupt_discontinuation_arm <- abrupt_discontinuation_arm %>%
    mutate(stabilized_weight = pmin(stabilized_weight_untruncated, quantile_cutoff))
  print("summary of truncated stabilized weights for abrupt_discontinuation arm:")
  print(summary(abrupt_discontinuation_arm$stabilized_weight))
  
  # truncate the unstabilized weights at the 99th percentile
  upper_bound_unstab <- quantile(abrupt_discontinuation_arm$unstabilized_weight_untruncated, probs=0.99)
  
  abrupt_discontinuation_arm <- abrupt_discontinuation_arm %>%
    mutate(unstabilized_weight = pmin(unstabilized_weight_untruncated, upper_bound_unstab))
  print("summary of truncated unstabilized weights for abrupt_discontinuation arm:")
  print(summary(abrupt_discontinuation_arm$unstabilized_weight))
  
  abrupt_discontinuation_arm <- abrupt_discontinuation_arm %>%
    mutate(uncensored = ifelse(no_discontinuation_censor == 0 & dose_reduction_censor == 0 & reinitiation_censor == 0,
                               1, 0)) %>%
  ...
  summary_abrupt <- do.call(rbind, lapply(vars, function(v) {
    stats <- get_summary_stats(abrupt_discontinuation_arm[[v]])
    data.frame(variable = v, t(stats))
  }))
  summary_abrupt <- summary_abrupt %>%
    mutate(arm = "abrupt discontinuation")
  ...
  summary_weights <- rbind(summary_continuation, summary_abrupt)
  
  fwrite(summary_weights, paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/weights_distribution/stroke.csv"))
  ...
  abrupt_discontinuation_t7 <- abrupt_discontinuation_arm %>%
    filter(t == 7 & uncensored == 1) %>%
    dplyr::select(patid, all_of(covariates), unstabilized_weight, treatment)
  
  
  combined_t7 <- rbind(continuation_t7, abrupt_discontinuation_t7)
  ...
  if(full_sample == TRUE){
    fwrite(table_one_df, paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/balance_checks/stroke.csv"))
    fwrite(pre_weighting_table, paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/balance_checks/pre_weighting_stroke.csv"))
  }
  ...
  abrupt_discontinuation_df <- abrupt_discontinuation_arm %>%
    dplyr::select(all_of(baseline_covariates), t, t_squared, treatment, outcome_date, outcome_month, outcome,
                  age_T0, gender, months_since_diagnosis, stabilized_weight, unstabilized_weight, censored)
  
  combined_data <- rbind(continuation_df, abrupt_discontinuation_df) %>%
    filter(censored == 0)
  ...
  fwrite(event_by_t,paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/number_at_risk/stroke.csv" ))
  ...
  abrupt_discontinuation_df <- long_format %>%
    filter(followup_curve == 1) %>%
    mutate(treatment = "discontinuation")
  
  # create hazards under each strategy
  abrupt_discontinuation_df$hazard1 <- predict(survival_curve_model, newdata=abrupt_discontinuation_df, type="response")
  ...
  abrupt_discontinuation_df <- abrupt_discontinuation_df %>%
    group_by(patid) %>%
  ...
  abrupt_discontinuation_risk <- abrupt_discontinuation_df %>%
  ...
  risk_curves <- full_join(abrupt_discontinuation_risk, continuation_risk)
  ...
  risk_curves_wider <- risk_curves_wider %>%
    mutate(hazard_ratio = log(survival_discontinuation)/log(survival_continuation))
  
  if(full_sample == TRUE){
    fwrite(risk_curves_wider, paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/outcome_model/stroke.csv"))
  }
  
  # hazard ratio is average of hazard ratios over time
  hazard_ratio_estimate <- mean(risk_curves_wider$hazard_ratio)
  
  abrupt_discontinuation_risk_12 <- abrupt_discontinuation_risk[abrupt_discontinuation_risk$t == 12,]$risk * 100
  continuation_risk_12 <- continuation_risk[continuation_risk$t == 12,]$risk * 100
  
  abrupt_discontinuation_risk_24 <- abrupt_discontinuation_risk[abrupt_discontinuation_risk$t == 24,]$risk * 100
  continuation_risk_24 <- continuation_risk[continuation_risk$t == 24,]$risk * 100
  
  # calculate risk ratios and differences
  risk_ratio_12 <- abrupt_discontinuation_risk_12/continuation_risk_12
  risk_diff_12 <- abrupt_discontinuation_risk_12-continuation_risk_12
  
  risk_ratio_24 <- abrupt_discontinuation_risk_24/continuation_risk_24
  risk_diff_24 <- abrupt_discontinuation_risk_24-continuation_risk_24
  ...
  risk_df <- data.frame(
    ...
    abs_risk_discontinuation_12 = abrupt_discontinuation_risk_12, 
    abs_risk_discontinuation_24 = abrupt_discontinuation_risk_24
  )
  rownames(risk_df) <- NULL
  
  if(full_sample == TRUE){
    fwrite(risk_df, paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/risk_estimates/stroke.csv"))
  }
  ...
  risk_curves <- rbind(
    ...
    data.frame(t = 0, risk = 0, treatment = "discontinuation")
  )
  ...
  if(full_sample == TRUE){
    ggsave(paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/survival_curves/stroke.png"), plot = survival_curve_plot,
...
abrupt_discontinuation_stroke_trials(combined_12weeks, '12weeks', full_sample = TRUE)
abrupt_discontinuation_stroke_trials(combined_24weeks, '24weeks', full_sample = TRUE)
...
abrupt_discontinuation_pneumonia_trials <- function(long_format, time_period, full_sample=FALSE) {
  ...
  fwrite(summary_weights, paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/weights_distribution/pneumonia.csv"))
  ...
  if(full_sample == TRUE){
    fwrite(table_one_df, paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/balance_checks/pneumonia.csv"))
    fwrite(pre_weighting_table, paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/balance_checks/pre_weighting_pneumonia.csv"))
  }
  ...
  fwrite(event_by_t,paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/number_at_risk/pneumonia.csv" ))
  ...
  if(full_sample == TRUE){
    fwrite(risk_curves_wider, paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/outcome_model/pneumonia.csv"))
  }
  ...
  if(full_sample == TRUE){
    fwrite(risk_df, paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/risk_estimates/pneumonia.csv"))
  }
  ...
  survival_curve_plot <- ggplot(risk_curves, aes(x = t, y = risk, color = treatment)) +
    ...
    geom_vline(xintercept = 6, 
               ...
               color = "black") 
  if(full_sample == TRUE){
    ggsave(paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/survival_curves/pneumonia.png"), plot = survival_curve_plot,
           dpi=300, width = 8, height = 6)
  }
...
abrupt_discontinuation_pneumonia_trials(combined_12weeks, '12weeks', full_sample = TRUE)
abrupt_discontinuation_pneumonia_trials(combined_24weeks, '24weeks', full_sample = TRUE)
...
abrupt_discontinuation_fracture_trials <- function(long_format, time_period, full_sample=FALSE) {
  ...
  fwrite(summary_weights, paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/weights_distribution/fracture.csv"))
  ...
  if(full_sample == TRUE){
    fwrite(table_one_df, paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/balance_checks/fracture.csv"))
    fwrite(pre_weighting_table, paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/balance_checks/pre_weighting_fracture.csv"))
  }
  ...
  fwrite(event_by_t,paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/number_at_risk/fracture.csv" ))
  ...
  if(full_sample == TRUE){
    fwrite(risk_curves_wider, paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/outcome_model/fracture.csv"))
  }
  ...
  rownames(risk_df) <- NULL
  if(full_sample == TRUE){
    fwrite(risk_df, paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/risk_estimates/fracture.csv"))
  }
  ...
  if(full_sample == TRUE){
    ggsave(paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/survival_curves/fracture.png"), plot = survival_curve_plot,
           dpi=300, width = 8, height = 6)
  }
...
abrupt_discontinuation_fracture_trials(combined_12weeks, '12weeks', full_sample = TRUE)
abrupt_discontinuation_fracture_trials(combined_24weeks, '24weeks', full_sample = TRUE)
...
abrupt_discontinuation_delirium_trials <- function(long_format, time_period, full_sample=FALSE) {
  ...
  fwrite(summary_weights, paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/weights_distribution/delirium.csv"))
  ...
  if(full_sample == TRUE){
    fwrite(table_one_df, paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/balance_checks/delirium.csv"))
    fwrite(pre_weighting_table, paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/balance_checks/pre_weighting_delirium.csv"))
  }
  ...
  fwrite(event_by_t,paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/number_at_risk/delirium.csv" ))
  ...
  if(full_sample == TRUE){
    fwrite(risk_curves_wider, paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/outcome_model/delirium.csv"))
  }
  ...
  if(full_sample == TRUE){
    fwrite(risk_df, paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/risk_estimates/delirium.csv"))
  }
  ...
  if(full_sample == TRUE){
    ggsave(paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/survival_curves/delirium.png"), plot = survival_curve_plot,
           dpi=300, width = 8, height = 6)
  }
...
abrupt_discontinuation_delirium_trials(combined_12weeks, '12weeks', full_sample = TRUE)
abrupt_discontinuation_delirium_trials(combined_24weeks, '24weeks', full_sample = TRUE)
...
abrupt_discontinuation_negcontrol_trials <- function(long_format, time_period, full_sample=FALSE) {
  ...
  fwrite(summary_weights, paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/weights_distribution/negcontrol.csv"))
  ...
  if(full_sample == TRUE){
    fwrite(table_one_df, paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/balance_checks/negcontrol.csv"))
    fwrite(pre_weighting_table, paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/balance_checks/pre_weighting_negcontrol.csv"))
  }
  ...
  fwrite(event_by_t,paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/number_at_risk/neg_control.csv" ))
  ...
  if(full_sample == TRUE){
    fwrite(risk_curves_wider, paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/outcome_model/negcontrol.csv"))
  }
  ...
  if(full_sample == TRUE){
    fwrite(risk_df, paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/risk_estimates/negcontrol.csv"))
  }
  ...
  survival_curve_plot <- ggplot(risk_curves, aes(x = t, y = risk, color = treatment)) +
    geom_line(linewidth=1)+
    labs(
      title = "Negative Control Cumulative Incidence Curves by Treatment",
      x = "Time (months)",
      y = "Cumulative Incidence (%)",
      color = "Treatment"
    ) +
  ...
  if(full_sample == TRUE){
    ggsave(paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/survival_curves/negcontrol.png"), plot = survival_curve_plot,
           dpi=300, width = 8, height = 6)
  }
...
abrupt_discontinuation_negcontrol_trials(combined_12weeks, '12weeks', full_sample = TRUE) 
...
abrupt_discontinuation_death_trials_12weeks <- function(long_format, time_period, full_sample=FALSE) {
  ...
  fwrite(summary_weights, paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/weights_distribution/death.csv"))
  ...
  if(full_sample == TRUE){
    fwrite(table_one_df, paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/balance_checks/death.csv"))
    fwrite(pre_weighting_table, paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/balance_checks/pre_weighting_death.csv"))
  }
  ...
  fwrite(event_by_t,paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/number_at_risk/death.csv" ))
  ...
  if(full_sample == TRUE){
    fwrite(risk_curves_wider, paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/outcome_model/death.csv"))
  }
  ...
  if(full_sample == TRUE){
    fwrite(risk_df, paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/risk_estimates/death.csv"))
  }
  ...
  if(full_sample == TRUE){
    ggsave(paste0("analysis_results/Abrupt Discontinuation Trials/", time_period, "/survival_curves/death.png"), plot = survival_curve_plot,
           dpi=300, width = 8, height = 6)
  }
...
abrupt_discontinuation_death_trials_24weeks <- function(long_format, time_period, full_sample=FALSE) {
...
abrupt_discontinuation_death_trials_12weeks(combined_12weeks, '12weeks', full_sample = TRUE)
abrupt_discontinuation_death_trials_24weeks(combined_24weeks, '24weeks', full_sample = TRUE)
...
# eligibility analyses for the abrupt discontinuation versus continuation comparison.
################################################################################
...
for (i in 1:n_boot){
  ...
  stroke_estimate <- abrupt_discontinuation_stroke_trials(sample_data, '12weeks', full_sample=FALSE)
  stroke_bootstrapped_estimates <- rbind(stroke_estimate, stroke_bootstrapped_estimates)
  
  pneumonia_estimate <- abrupt_discontinuation_pneumonia_trials(sample_data, '12weeks', full_sample=FALSE)
  pneumonia_bootstrapped_estimates <- rbind(pneumonia_estimate, pneumonia_bootstrapped_estimates)
  
  fracture_estimate <- abrupt_discontinuation_fracture_trials(sample_data, '12weeks', full_sample=FALSE)
  fracture_bootstrapped_estimates <- rbind(fracture_estimate, fracture_bootstrapped_estimates)
  
  death_estimate <- abrupt_discontinuation_death_trials_12weeks(sample_data, '12weeks', full_sample=FALSE)
  death_bootstrapped_estimates_12weeks <- rbind(death_estimate, death_bootstrapped_estimates_12weeks)
  
  delirium_estimate <- abrupt_discontinuation_delirium_trials(sample_data, '12weeks', full_sample=FALSE)
  ...
  fwrite(stroke_bootstrapped_estimates, 
         "analysis_results/Abrupt Discontinuation Trials/12weeks/bootstrap_results/bootstrapped_stroke.csv", row.names=FALSE)
  fwrite(pneumonia_bootstrapped_estimates, 
         "analysis_results/Abrupt Discontinuation Trials/12weeks/bootstrap_results/bootstrapped_pneumonia.csv", row.names=FALSE)
  fwrite(fracture_bootstrapped_estimates, 
         "analysis_results/Abrupt Discontinuation Trials/12weeks/bootstrap_results/bootstrapped_fracture.csv", row.names=FALSE)
  fwrite(death_bootstrapped_estimates_12weeks, 
         "analysis_results/Abrupt Discontinuation Trials/12weeks/bootstrap_results/bootstrapped_death.csv", row.names=FALSE)
  fwrite(delirium_bootstrapped_estimates,
         "analysis_results/Abrupt Discontinuation Trials/12weeks/bootstrap_results/bootstrapped_delirium.csv", row.names=FALSE)
...
for (i in 1:n_boot){
  ...
  stroke_estimate <- abrupt_discontinuation_stroke_trials(sample_data, '24weeks', full_sample=FALSE)
  stroke_bootstrapped_estimates <- rbind(stroke_estimate, stroke_bootstrapped_estimates)
  
  pneumonia_estimate <- abrupt_discontinuation_pneumonia_trials(sample_data, '24weeks', full_sample=FALSE)
  pneumonia_bootstrapped_estimates <- rbind(pneumonia_estimate, pneumonia_bootstrapped_estimates)
  
  fracture_estimate <- abrupt_discontinuation_fracture_trials(sample_data, '24weeks', full_sample=FALSE)
  fracture_bootstrapped_estimates <- rbind(fracture_estimate, fracture_bootstrapped_estimates)
  
  death_estimate <- abrupt_discontinuation_death_trials_24weeks(sample_data, '24weeks', full_sample=FALSE)
  death_bootstrapped_estimates_24weeks <- rbind(death_estimate, death_bootstrapped_estimates_24weeks)
  
  delirium_estimate <- abrupt_discontinuation_delirium_trials(sample_data, '24weeks', full_sample=FALSE)
  delirium_bootstrapped_estimates <- rbind(delirium_estimate, delirium_bootstrapped_estimates)
  
  fwrite(stroke_bootstrapped_estimates, 
         "analysis_results/Abrupt Discontinuation Trials/24weeks/bootstrap_results/bootstrapped_stroke.csv", row.names=FALSE)
  fwrite(pneumonia_bootstrapped_estimates, 
        "analysis_results/Abrupt Discontinuation Trials/24weeks/bootstrap_results/bootstrapped_pneumonia.csv", row.names=FALSE)
  fwrite(fracture_bootstrapped_estimates, 
         "analysis_results/Abrupt Discontinuation Trials/24weeks/bootstrap_results/bootstrapped_fracture.csv", row.names=FALSE)
  fwrite(death_bootstrapped_estimates_24weeks, 
         "analysis_results/Abrupt Discontinuation Trials/24weeks/bootstrap_results/bootstrapped_death.csv", row.names=FALSE)
  fwrite(delirium_bootstrapped_estimates,
       "analysis_results/Abrupt Discontinuation Trials/24weeks/bootstrap_results/bootstrapped_delirium.csv", row.names=FALSE)
